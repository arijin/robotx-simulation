...
void LivoxPointsPlugin::OnNewLaserScans() {
    if (rayShape) {
        ...
        ROS_INFO_STREAM("frame show:-------------------");
        ROS_INFO_STREAM(world->Name() << " " << raySensor->Name() << " " << raySensor->ParentName());
...
void LivoxPointsPlugin::SendRosTf(const ignition::math::Pose3d &pose, const std::string &father_frame,
    ...
    tfBroadcaster->sendTransform(
        tf::StampedTransform(tf, ros::Time::now(), "livox_link", raySensor->Name()));//raySensor->ParentName()
}
